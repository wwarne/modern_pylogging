...
from modern_pylogging.picologging_handlers import PicoStreamHandlerToStdout
...
def test_correct_dict_config_called(
    ...
    modern_pylogging.LoggingConfig(logging_module=logging_module).configure()
...
def test_correct_default_handlers_set(
    picologging_installed: bool,
    expected_default_handlers: dict[str, dict[str, Any]],
    mocker: MockerFixture,
) -> None:
    mock = mocker.patch('modern_pylogging.config_api.import_checker')
    mock.is_picologging_installed = picologging_installed
    log_config = modern_pylogging.LoggingConfig()._prepare_config_dict()
...
@pytest.mark.parametrize(
    'logging_module, expected_handlers', [('picologging', default_picologging_handlers), ('logging', default_handlers)]
)
def test_correct_handlers_set_by_param(
    logging_module: str,
    expected_handlers: dict[str, dict[str, Any]],
) -> None:
    log_config = modern_pylogging.LoggingConfig(logging_module=logging_module)._prepare_config_dict()
...
def test_correct_json_module_set_by_param(
    ...
    log_config = modern_pylogging.LoggingConfig(json_dumps_module=json_module_name)._prepare_config_dict()
    assert log_config['formatters']['json_fmt']['json_dumps_module'] == json_module_name


def test_capture_extra_fields_cannot_be_used_with_picologging() -> None:
    log_config = modern_pylogging.LoggingConfig(
        logging_module='picologging', capture_extra_fields=True
    )._prepare_config_dict()
    assert log_config['formatters']['json_fmt']['capture_extra_fields'] == False
...
def test_default_queue_listener_handler(
    ...
    get_logger = modern_pylogging.LoggingConfig(
...
def test_default_logger_and_handler(logging_module: ModuleType, expected_handler_class: Any) -> None:
    get_logger = modern_pylogging.LoggingConfig(logging_module=logging_module.__name__).configure()
...
def test_customizing_handler(
    ...
    assert type(root_handler) is expected_handler_class

    if type(root_handler) is logging_handlers.QueueHandlerContextVarsHappyPy312:
        root_formatter = root_handler.listener.handlers[0].formatter
    else:
    ...
    assert type(t_logger) is logging_module.Logger
    assert t_logger.level == logging_module.DEBUG
    assert len(t_logger.handlers) == 1
    if logging_module == picologging:
        assert type(t_logger.handlers[0]) is PicoStreamHandlerToStdout
    else:
        assert type(t_logger.handlers[0]) is logging_module.StreamHandler
...
def test_excluded_fields(logging_module: ModuleType, mocker: MockerFixture) -> None:
    ...
    modern_pylogging.LoggingConfig(logging_module=logging_module.__name__).configure()
...
def test_default_queue_listener_handler_json_fmt_extra(capsys: CaptureFixture[str]) -> None:
    """
